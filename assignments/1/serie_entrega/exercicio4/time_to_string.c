 ...
 * 2. Verifica se o buffer fornecido é suficiente para armazenar a string
 * resultante.
...
struct tm {
  int tm_sec;   // segundos depois do minuto - [0, 60]
  int tm_min;   // minutos depois da hora - [0, 59]
  int tm_hour;  // houras desde a meia-noite - [0, 23]
  ...
  int tm_isdst; // flag de hora dee inverno
};

size_t time_to_string(struct tm *tm, char *buffer, size_t buffer_size) {
  const char *week_day[] = {"domingo",      "segunda-feira", "terça-feira",
  ...
  required_size +=
      strlen(week_day[tm->tm_wday]) + 2; // dia da semana + virgula + espaço
  ...
  if (index < buffer_size - 1)
    buffer[index++] = (year / 1000) + '0';
  if (index < buffer_size - 1)
    buffer[index++] = ((year / 100) % 10) + '0';
  if (index < buffer_size - 1)
    buffer[index++] = ((year / 10) % 10) + '0';
  if (index < buffer_size - 1)
    buffer[index++] = (year % 10) + '0';
